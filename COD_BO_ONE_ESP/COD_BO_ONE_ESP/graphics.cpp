...
void DrawRadar(IDirect3DDevice9* pDevice, const Player& player, const std::vector<Entity>& entities, float radarRadius, float radarCenterX, float radarCenterY)
{
    ...
    */
    ...
    for (const auto& entity : entities)
    {
        ...
        float radarX = radarCenterX + (relativePos.x / 10.0f);  // Scale the x-coordinate for the radar
        float radarY = radarCenterY - (relativePos.z / 10.0f);  // Scale the y-coordinate for the radar

        // Calculate the distance to the enemy (convert from centimeters to meters)
        float distance = sqrtf(relativePos.x * relativePos.x + relativePos.y * relativePos.y + relativePos.z * relativePos.z) / 100.0f;
...
void renderFrame(D3DContext& d3dContext, uintptr_t& moduleBase, HANDLE hProc)
{
    ...
    uintptr_t viewMatrixAddr = moduleBase + 0x420F758;
    ViewMatrix viewMatrix = Memory::RPM<ViewMatrix>(hProc, viewMatrixAddr);

    uintptr_t entitylist = moduleBase + 0x17FBD10;

    Player player;
    uintptr_t localplayer = moduleBase + 0x01653770;
    player.baseAdress = Memory::RPM<uintptr_t>(hProc, localplayer);
    player.Origin = Memory::RPM<Vec3>(hProc, player.baseAdress + 0x24);
    player.Angles.X = Memory::RPM<float>(hProc, player.baseAdress + 0x180);
    player.Angles.Y = Memory::RPM<float>(hProc, player.baseAdress + 0x180 + 0x4);

    std::vector<Entity> entity(120);

    for (size_t i = 0; i < 120; ++i)
    {
        entity[i].baseAdress = Memory::RPM<uintptr_t>(hProc, entitylist + (i * 0x8C));
        entity[i].health = Memory::RPM<int>(hProc, entity[i].baseAdress + 0x184);
        entity[i].maxHealth = Memory::RPM<int>(hProc, entity[i].baseAdress + 0x188);
        entity[i].Origin = Memory::RPM<Vec3>(hProc, entity[i].baseAdress + 0x18);

        if (entity[i].health <= 0)
            continue;
    }

    // Draw the radar at a fixed position (top-left corner)
    float radarCenterX = 150.0f;
    float radarCenterY = 150.0f;
    float radarRadius = 100.0f;
    DrawRadar(d3dContext.d3ddev, player, entity, radarRadius, radarCenterX, radarCenterY);
